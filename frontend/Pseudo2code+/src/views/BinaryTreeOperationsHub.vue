...
<style scoped>
.bt-page{min-height:100vh;background:var(--bg-page);color:var(--text-secondary);padding:16px 24px 40px;font-family:'Inter','Segoe UI',sans-serif}
...
.bt-title{font-size:1.6rem;font-weight:700;color:var(--text-primary);margin:0 0 4px}
.bt-subtitle{color:var(--text-muted);font-size:.9rem;margin:0 0 16px}

/* Operation pills */
.bt-op-pills{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px}
.bt-pill{padding:7px 16px;border-radius:20px;border: 1px solid var(--border-default);background:rgba(100,116,139,.1);color:var(--text-tertiary);font-size:.82rem;font-weight:500;cursor:pointer;transition:all .15s;white-space:nowrap}
.bt-pill:hover{background:var(--accent-bg);border-color: var(--accent-border)}
.bt-pill.active{background:var(--accent-bg);border-color: var(--accent-border);color:var(--accent-lighter);font-weight:600}

/* Three-column grid */
.bt-three-col{display:grid;grid-template-columns:260px 1fr 280px;gap:16px;margin-bottom:24px}
.bt-controls-panel{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:14px;display:flex;flex-direction:column;gap:12px}
.bt-btn-group{display:grid;grid-template-columns:1fr 1fr;gap:6px}
.bt-btn{display:flex;align-items:center;gap:6px;padding:8px 10px;border-radius:8px;border: 1px solid var(--border-default);background:rgba(100,116,139,.12);color:var(--text-tertiary);font-size:.82rem;font-weight:500;cursor:pointer;transition:all .15s}
.bt-btn:hover:not(:disabled){background:rgba(100,116,139,.25)}
.bt-btn:disabled{opacity:.4;cursor:not-allowed}
.bt-btn.active{background:var(--accent-bg);border-color: var(--accent-border)}
.bt-icon{font-size:.9rem}
.bt-settings-toggle{width:100%;justify-content:center}
.bt-settings-body{display:flex;flex-direction:column;gap:8px;padding:8px;background:var(--bg-elevated);border-radius:8px}
.bt-setting-row label{font-size:.78rem;color:var(--text-muted)}
.bt-slider{width:100%;cursor:pointer;accent-color: var(--accent-light)}
.bt-custom-input{display:flex;flex-direction:column;gap:4px}
.bt-custom-input label{font-size:.75rem;color:var(--text-muted);font-weight:600}
.bt-input-row{display:flex;gap:4px}
.bt-text-input{flex:1;background:var(--bg-input);border: 1px solid var(--border-default);color:var(--accent-lighter);padding:6px 10px;border-radius:6px;font-size:.8rem;outline:none;width:100%}
.bt-text-input:focus{border-color: var(--accent)}
.bt-text-input::placeholder{color:var(--text-faint);font-size:.72rem}
.bt-apply-btn{padding:6px 12px!important;background: var(--accent)!important;border:none!important;color: var(--text-primary)!important;font-weight:600!important}
.bt-op-params{display:flex;flex-direction:column;gap:6px}
.bt-param-label{font-size:.75rem;color:var(--text-muted);font-weight:600}
.bt-code-btn{width:100%;justify-content:center;background:var(--accent-bg)!important;border-color: var(--accent-border)!important;color:var(--accent-lighter)!important}
.bt-code-btn:hover{background:var(--accent-bg)!important}
.bt-shortcuts h4{font-size:.78rem;color:var(--text-muted);margin:0 0 6px;font-weight:600}
.bt-shortcut-grid{display:grid;grid-template-columns:auto 1fr;gap:4px 8px;font-size:.75rem;color:var(--text-muted)}
.bt-key{background:rgba(100,116,139,.25);padding:2px 6px;border-radius:4px;font-family:monospace;color:var(--accent-lighter);font-size:.72rem;text-align:center}
.bt-legend h4{font-size:.82rem;color:var(--text-primary);margin:0 0 8px}
.bt-legend-item{display:flex;gap:8px;align-items:flex-start;margin-bottom:8px;font-size:.78rem;color:var(--text-tertiary)}
.bt-legend-item small{color:var(--text-dim)}
...
.bt-dot.default{background-color: var(--accent)}

/* Center chart area */
.bt-chart-area{display:flex;flex-direction:column;gap:10px}
.bt-chart-wrapper{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:24px 16px 8px;display:flex;flex-direction:column;align-items:center;min-height:280px}
...
.bt-empty-msg{color:var(--text-dim);font-size:.9rem;padding:40px 0;font-style:italic}
...
.bt-chart-footer{font-size:.72rem;color:var(--text-dim);padding:6px 0 4px;text-align:center;width:100%}
.bt-scrubber{width:100%;accent-color: var(--accent-light);cursor:pointer;margin-top:4px}
.bt-status-bar{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:10px;padding:12px 20px;text-align:center;font-size:.9rem;color:var(--text-secondary);font-weight:500}
.bt-complexity-row{display:flex;justify-content:center;gap:32px}
.bt-complexity-badge{font-size:.82rem;color:var(--text-muted)}
.bt-complexity-badge strong{color:var(--accent-lighter)}
.bt-traversal-result{background:var(--bg-card);border: 1px solid var(--accent-border);border-radius:10px;padding:10px 20px;text-align:center;font-size:.85rem;color:var(--accent-lighter);font-family:'Fira Code',monospace}

/* Inspector */
.bt-inspector{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:14px}
.bt-inspector-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}
.bt-inspector-header h3{margin:0;font-size:1rem;color:var(--text-primary)}
.bt-step-badge{background:var(--accent-bg);border: 1px solid var(--accent-border);padding:3px 10px;border-radius:12px;font-size:.72rem;color:var(--accent-lighter);font-weight:600}
.bt-inspector-row{display:flex;justify-content:space-between;font-size:.78rem;color:var(--text-muted);margin-bottom:4px}
...
.bt-progress-fill{height:100%;background: var(--accent);border-radius:3px;transition:width .25s}
.bt-inspector-label{font-size:.7rem;letter-spacing:.06em;color:var(--text-dim);margin:14px 0 6px;font-weight:700}
.bt-op-item{display:flex;align-items:center;gap:8px;font-size:.78rem;color:var(--text-tertiary);margin-bottom:4px}
...
.bt-metric-label{font-size:.7rem;color:var(--text-dim)}
.bt-metric-value{font-size:1rem;font-weight:700;color:var(--text-primary)}
.bt-step-detail{background:var(--bg-elevated);padding:8px 10px;border-radius:6px;font-size:.78rem;color:var(--text-tertiary);font-family:'Fira Code',monospace}
.bt-tree-state{background:var(--bg-elevated);padding:8px 10px;border-radius:6px;font-size:.72rem;color:var(--text-tertiary);font-family:'Fira Code',monospace;word-break:break-all;line-height:1.5}
.bt-tree-info{font-size:.7rem;color:var(--text-dim);margin-top:4px}
.bt-active-ptr{font-size:.7rem;color:#22c55e;margin-top:2px}

/* Sections */
.bt-section{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;margin-bottom:12px;overflow:hidden}
.bt-section-toggle{width:100%;display:flex;align-items:center;gap:10px;padding:14px 18px;background:none;border:none;color:var(--text-secondary);font-size:1rem;font-weight:600;cursor:pointer;text-align:left;transition:background .15s}
.bt-section-toggle:hover{background:rgba(100,116,139,.1)}
.bt-info-circle{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:var(--accent-bg);border: 1px solid var(--accent-border);color:var(--accent-lighter);font-size:.82rem;font-weight:700;flex-shrink:0}
.bt-section-body{padding:0 20px 20px;color:var(--text-tertiary);font-size:.88rem;line-height:1.7}
.bt-section-body h2{font-size:1.15rem;color:var(--text-primary);margin:0 0 8px}
.bt-section-body h3{font-size:.95rem;color:var(--accent-lighter);margin:16px 0 6px}
.bt-section-body ol,.bt-section-body ul{padding-left:20px;margin:4px 0}
.bt-section-body li{margin-bottom:3px}
.bt-section-intro{color:var(--text-muted);margin:0 0 16px;font-size:.9rem}
.bt-how-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px;margin-bottom:12px}
.bt-how-card{background:var(--bg-input);border: 1px solid var(--border-default);border-radius:12px;padding:16px;display:flex;flex-direction:column;gap:12px;min-height:220px;transition:border .2s,transform .2s}
.bt-how-card.active{border-color: var(--accent-border);transform:translateY(-4px);box-shadow:0 8px 24px rgba(99,102,241,.2)}
.bt-how-header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}
.bt-how-header h3{margin:0;color:var(--text-primary);font-size:1rem}
.bt-how-header p{margin:4px 0 0;color:var(--text-tertiary);font-size:.85rem}
.bt-chip{background:var(--accent-bg);border: 1px solid var(--accent-border);border-radius:999px;padding:3px 10px;font-size:.7rem;color:var(--accent-lighter);font-weight:600}
.bt-step-list{margin:0;padding-left:18px;color:var(--text-tertiary);font-size:.84rem;line-height:1.5}
.bt-how-meta{display:flex;flex-wrap:wrap;gap:8px;font-size:.75rem;color:var(--text-muted)}
.bt-how-meta strong{color:var(--text-primary)}
.bt-edge-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px}
.bt-edge-card{background:var(--bg-elevated);border: 1px solid var(--border-default);border-radius:10px;padding:14px;cursor:pointer;transition:all .15s}
.bt-edge-card:hover{border-color: var(--accent-border);background:var(--accent-bg)}
.bt-edge-card strong{display:block;color:var(--text-primary);font-size:.88rem;margin-bottom:4px}
.bt-edge-card small{color:var(--text-dim);font-size:.78rem}
