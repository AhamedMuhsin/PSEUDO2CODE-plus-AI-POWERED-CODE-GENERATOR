...
<style scoped>
.qo-page{min-height:100vh;background:var(--bg-page);color:var(--text-secondary);padding:16px 24px 40px;font-family:'Inter','Segoe UI',sans-serif}
...
.qo-title{font-size:1.6rem;font-weight:700;color:var(--text-primary);margin:0 0 4px}
.qo-subtitle{color:var(--text-muted);font-size:.9rem;margin:0 0 16px}

/* Operation pills */
.qo-op-pills{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px}
.qo-pill{padding:7px 16px;border-radius:20px;border: 1px solid var(--border-default);background:rgba(100,116,139,.1);color:var(--text-tertiary);font-size:.82rem;font-weight:500;cursor:pointer;transition:all .15s;white-space:nowrap}
.qo-pill:hover{background:var(--accent-bg);border-color: var(--accent-border)}
.qo-pill.active{background:var(--accent-bg);border-color: var(--accent-border);color:var(--accent-lighter);font-weight:600}

/* Three-column grid */
.qo-three-col{display:grid;grid-template-columns:260px 1fr 280px;gap:16px;margin-bottom:24px}
.qo-controls-panel{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:14px;display:flex;flex-direction:column;gap:12px}
.qo-btn-group{display:grid;grid-template-columns:1fr 1fr;gap:6px}
.qo-btn{display:flex;align-items:center;gap:6px;padding:8px 10px;border-radius:8px;border: 1px solid var(--border-default);background:rgba(100,116,139,.12);color:var(--text-tertiary);font-size:.82rem;font-weight:500;cursor:pointer;transition:all .15s}
.qo-btn:hover:not(:disabled){background:rgba(100,116,139,.25)}
.qo-btn:disabled{opacity:.4;cursor:not-allowed}
.qo-btn.active{background:var(--accent-bg);border-color: var(--accent-border)}
.qo-icon{font-size:.9rem}
.qo-settings-toggle{width:100%;justify-content:center}
.qo-settings-body{display:flex;flex-direction:column;gap:8px;padding:8px;background:var(--bg-elevated);border-radius:8px}
.qo-setting-row label{font-size:.78rem;color:var(--text-muted)}
.qo-slider{width:100%;cursor:pointer;accent-color: var(--accent-light)}
.qo-custom-input{display:flex;flex-direction:column;gap:4px}
.qo-custom-input label{font-size:.75rem;color:var(--text-muted);font-weight:600}
.qo-input-row{display:flex;gap:4px}
.qo-text-input{flex:1;background:var(--bg-input);border: 1px solid var(--border-default);color:var(--accent-lighter);padding:6px 10px;border-radius:6px;font-size:.8rem;outline:none;width:100%}
.qo-text-input:focus{border-color: var(--accent)}
.qo-text-input::placeholder{color:var(--text-faint);font-size:.72rem}
.qo-apply-btn{padding:6px 12px!important;background: var(--accent)!important;border:none!important;color: var(--text-primary)!important;font-weight:600!important}
.qo-op-params{display:flex;flex-direction:column;gap:6px}
.qo-param-label{font-size:.75rem;color:var(--text-muted);font-weight:600}
.qo-code-btn{width:100%;justify-content:center;background:var(--accent-bg)!important;border-color: var(--accent-border)!important;color:var(--accent-lighter)!important}
.qo-code-btn:hover{background:var(--accent-bg)!important}
.qo-shortcuts h4{font-size:.78rem;color:var(--text-muted);margin:0 0 6px;font-weight:600}
.qo-shortcut-grid{display:grid;grid-template-columns:auto 1fr;gap:4px 8px;font-size:.75rem;color:var(--text-muted)}
.qo-key{background:rgba(100,116,139,.25);padding:2px 6px;border-radius:4px;font-family:monospace;color:var(--accent-lighter);font-size:.72rem;text-align:center}
.qo-legend h4{font-size:.82rem;color:var(--text-primary);margin:0 0 8px}
.qo-legend-item{display:flex;gap:8px;align-items:flex-start;margin-bottom:8px;font-size:.78rem;color:var(--text-tertiary)}
.qo-legend-item small{color:var(--text-dim)}
...
.qo-dot.default{background-color: var(--accent)}

/* Center chart area */
.qo-chart-area{display:flex;flex-direction:column;gap:10px}
.qo-chart-wrapper{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:24px 16px 8px;display:flex;flex-direction:column;align-items:center;min-height:220px}
...
.qo-empty-msg{color:var(--text-dim);font-size:.9rem;padding:40px 0;font-style:italic}
...
.qo-pointer.both{background:rgba(168,85,247,.2);color: var(--accent-lighter);border:1px solid rgba(168,85,247,.4)}
.qo-pointer-spacer{display:block;height:22px}
.qo-cell{min-width:52px;padding:14px 8px;border-radius:10px;background-color: var(--accent);color: var(--text-primary);font-weight:700;font-size:1rem;text-align:center;transition:all .3s ease;box-shadow:0 2px 8px rgba(99,102,241,.25)}
...
.qo-cell.front-back{background: var(--accent);box-shadow:0 0 14px rgba(168,85,247,.3)}
.qo-cell-index{font-size:.6rem;color:var(--text-dim)}
.qo-flow-arrow{font-size:.65rem;color:var(--text-faint);letter-spacing:.12em;margin-top:2px}
.qo-chart-footer{font-size:.72rem;color:var(--text-dim);padding:6px 0 4px;text-align:center;width:100%}
.qo-scrubber{width:100%;accent-color: var(--accent-light);cursor:pointer;margin-top:4px}
.qo-status-bar{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:10px;padding:12px 20px;text-align:center;font-size:.9rem;color:var(--text-secondary);font-weight:500}
.qo-complexity-row{display:flex;justify-content:center;gap:32px}
.qo-complexity-badge{font-size:.82rem;color:var(--text-muted)}
.qo-complexity-badge strong{color:var(--accent-lighter)}

/* Inspector */
.qo-inspector{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:14px}
.qo-inspector-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}
.qo-inspector-header h3{margin:0;font-size:1rem;color:var(--text-primary)}
.qo-step-badge{background:var(--accent-bg);border: 1px solid var(--accent-border);padding:3px 10px;border-radius:12px;font-size:.72rem;color:var(--accent-lighter);font-weight:600}
.qo-inspector-row{display:flex;justify-content:space-between;font-size:.78rem;color:var(--text-muted);margin-bottom:4px}
...
.qo-progress-fill{height:100%;background: var(--accent);border-radius:3px;transition:width .25s}
.qo-inspector-label{font-size:.7rem;letter-spacing:.06em;color:var(--text-dim);margin:14px 0 6px;font-weight:700}
.qo-op-item{display:flex;align-items:center;gap:8px;font-size:.78rem;color:var(--text-tertiary);margin-bottom:4px}
...
.qo-metric-label{font-size:.7rem;color:var(--text-dim)}
.qo-metric-value{font-size:1rem;font-weight:700;color:var(--text-primary)}
.qo-step-detail{background:var(--bg-elevated);padding:8px 10px;border-radius:6px;font-size:.78rem;color:var(--text-tertiary);font-family:'Fira Code',monospace}
.qo-queue-state{background:var(--bg-elevated);padding:8px 10px;border-radius:6px;font-size:.72rem;color:var(--text-tertiary);font-family:'Fira Code',monospace;word-break:break-all;line-height:1.5}
.qo-queue-length{font-size:.7rem;color:var(--text-dim);margin-top:4px}
.qo-queue-pointers{font-size:.7rem;color:var(--text-dim);margin-top:2px}

/* Sections */
.qo-section{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;margin-bottom:12px;overflow:hidden}
.qo-section-toggle{width:100%;display:flex;align-items:center;gap:10px;padding:14px 18px;background:none;border:none;color:var(--text-secondary);font-size:1rem;font-weight:600;cursor:pointer;text-align:left;transition:background .15s}
.qo-section-toggle:hover{background:rgba(100,116,139,.1)}
.qo-info-circle{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:var(--accent-bg);border: 1px solid var(--accent-border);color:var(--accent-lighter);font-size:.82rem;font-weight:700;flex-shrink:0}
.qo-section-body{padding:0 20px 20px;color:var(--text-tertiary);font-size:.88rem;line-height:1.7}
.qo-section-body h2{font-size:1.15rem;color:var(--text-primary);margin:0 0 8px}
.qo-section-body h3{font-size:.95rem;color:var(--accent-lighter);margin:16px 0 6px}
.qo-section-body ol,.qo-section-body ul{padding-left:20px;margin:4px 0}
.qo-section-body li{margin-bottom:3px}
.qo-section-intro{color:var(--text-muted);margin:0 0 16px;font-size:.9rem}
.qo-how-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px}
.qo-how-card{background:var(--bg-input);border: 1px solid var(--border-default);border-radius:12px;padding:16px;display:flex;flex-direction:column;gap:12px;min-height:220px;transition:border .2s,transform .2s}
.qo-how-card.active{border-color: var(--accent-border);transform:translateY(-4px);box-shadow:0 8px 24px rgba(99,102,241,.2)}
.qo-how-header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}
.qo-how-header h3{margin:0;color:var(--text-primary);font-size:1rem}
.qo-how-header p{margin:4px 0 0;color:var(--text-tertiary);font-size:.85rem}
.qo-chip{background:var(--accent-bg);border: 1px solid var(--accent-border);border-radius:999px;padding:3px 10px;font-size:.7rem;color:var(--accent-lighter);font-weight:600}
.qo-step-list{margin:0;padding-left:18px;color:var(--text-tertiary);font-size:.84rem;line-height:1.5}
.qo-how-meta{display:flex;flex-wrap:wrap;gap:8px;font-size:.75rem;color:var(--text-muted)}
.qo-how-meta strong{color:var(--text-primary)}
.qo-edge-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px}
.qo-edge-card{background:var(--bg-elevated);border: 1px solid var(--border-default);border-radius:10px;padding:14px;cursor:pointer;transition:all .15s}
.qo-edge-card:hover{border-color: var(--accent-border);background:var(--accent-bg)}
.qo-edge-card strong{display:block;color:var(--text-primary);font-size:.88rem;margin-bottom:4px}
.qo-edge-card small{color:var(--text-dim);font-size:.78rem}
