...
<style scoped>
.ao-page{min-height:100vh;background:var(--bg-page);color:var(--text-secondary);padding:16px 24px 40px;font-family:'Inter','Segoe UI',sans-serif}
...
.ao-title{font-size:1.6rem;font-weight:700;color:var(--text-primary);margin:0 0 4px}
.ao-subtitle{color:var(--text-muted);font-size:.9rem;margin:0 0 16px}

/* Operation pills */
.ao-op-pills{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px}
.ao-pill{padding:7px 16px;border-radius:20px;border: 1px solid var(--border-default);background:rgba(100,116,139,.1);color:var(--text-tertiary);font-size:.82rem;font-weight:500;cursor:pointer;transition:all .15s;white-space:nowrap}
.ao-pill:hover{background:var(--accent-bg);border-color: var(--accent-border)}
.ao-pill.active{background:var(--accent-bg);border-color: var(--accent-border);color:var(--accent-lighter);font-weight:600}

/* Three-column grid */
.ao-three-col{display:grid;grid-template-columns:260px 1fr 280px;gap:16px;margin-bottom:24px}
.ao-controls-panel{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:14px;display:flex;flex-direction:column;gap:12px}
.ao-btn-group{display:grid;grid-template-columns:1fr 1fr;gap:6px}
.ao-btn{display:flex;align-items:center;gap:6px;padding:8px 10px;border-radius:8px;border: 1px solid var(--border-default);background:rgba(100,116,139,.12);color:var(--text-tertiary);font-size:.82rem;font-weight:500;cursor:pointer;transition:all .15s}
.ao-btn:hover:not(:disabled){background:rgba(100,116,139,.25)}
.ao-btn:disabled{opacity:.4;cursor:not-allowed}
.ao-btn.active{background:var(--accent-bg);border-color: var(--accent-border)}
.ao-icon{font-size:.9rem}
.ao-settings-toggle{width:100%;justify-content:center}
.ao-settings-body{display:flex;flex-direction:column;gap:8px;padding:8px;background:var(--bg-elevated);border-radius:8px}
.ao-setting-row label{font-size:.78rem;color:var(--text-muted)}
.ao-slider{width:100%;cursor:pointer;accent-color: var(--accent-light)}
.ao-custom-input{display:flex;flex-direction:column;gap:4px}
.ao-custom-input label{font-size:.75rem;color:var(--text-muted);font-weight:600}
.ao-input-row{display:flex;gap:4px}
.ao-text-input{flex:1;background:var(--bg-input);border: 1px solid var(--border-default);color:var(--accent-lighter);padding:6px 10px;border-radius:6px;font-size:.8rem;outline:none;width:100%}
.ao-text-input:focus{border-color: var(--accent)}
.ao-text-input::placeholder{color:var(--text-faint);font-size:.72rem}
.ao-apply-btn{padding:6px 12px!important;background: var(--accent)!important;border:none!important;color: var(--text-primary)!important;font-weight:600!important}
.ao-op-params{display:flex;flex-direction:column;gap:6px}
.ao-param-label{font-size:.75rem;color:var(--text-muted);font-weight:600}
.ao-code-btn{width:100%;justify-content:center;background:var(--accent-bg)!important;border-color: var(--accent-border)!important;color:var(--accent-lighter)!important}
.ao-code-btn:hover{background:var(--accent-bg)!important}
.ao-shortcuts h4{font-size:.78rem;color:var(--text-muted);margin:0 0 6px;font-weight:600}
.ao-shortcut-grid{display:grid;grid-template-columns:auto 1fr;gap:4px 8px;font-size:.75rem;color:var(--text-muted)}
.ao-key{background:rgba(100,116,139,.25);padding:2px 6px;border-radius:4px;font-family:monospace;color:var(--accent-lighter);font-size:.72rem;text-align:center}
.ao-legend h4{font-size:.82rem;color:var(--text-primary);margin:0 0 8px}
.ao-legend-item{display:flex;gap:8px;align-items:flex-start;margin-bottom:8px;font-size:.78rem;color:var(--text-tertiary)}
.ao-legend-item small{color:var(--text-dim)}
...
.ao-dot.default{background-color: var(--accent)}

/* Center chart area */
.ao-chart-area{display:flex;flex-direction:column;gap:10px}
.ao-chart-wrapper{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:24px 16px 8px;display:flex;flex-direction:column;min-height:220px}
...
.ao-cell{min-width:52px;padding:14px 8px;border-radius:10px;background-color: var(--accent);color: var(--text-primary);font-weight:700;font-size:1rem;text-align:center;transition:all .3s ease;box-shadow:0 2px 8px rgba(99,102,241,.25)}
...
.ao-cell-index{font-size:.6rem;color:var(--text-dim)}
.ao-chart-footer{font-size:.72rem;color:var(--text-dim);padding:6px 0 4px;text-align:center}
.ao-scrubber{width:100%;accent-color: var(--accent-light);cursor:pointer;margin-top:4px}
.ao-status-bar{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:10px;padding:12px 20px;text-align:center;font-size:.9rem;color:var(--text-secondary);font-weight:500}
.ao-complexity-row{display:flex;justify-content:center;gap:32px}
.ao-complexity-badge{font-size:.82rem;color:var(--text-muted)}
.ao-complexity-badge strong{color:var(--accent-lighter)}

/* Inspector */
.ao-inspector{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;padding:14px}
.ao-inspector-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}
.ao-inspector-header h3{margin:0;font-size:1rem;color:var(--text-primary)}
.ao-step-badge{background:var(--accent-bg);border: 1px solid var(--accent-border);padding:3px 10px;border-radius:12px;font-size:.72rem;color:var(--accent-lighter);font-weight:600}
.ao-inspector-row{display:flex;justify-content:space-between;font-size:.78rem;color:var(--text-muted);margin-bottom:4px}
...
.ao-progress-fill{height:100%;background: var(--accent);border-radius:3px;transition:width .25s}
.ao-inspector-label{font-size:.7rem;letter-spacing:.06em;color:var(--text-dim);margin:14px 0 6px;font-weight:700}
.ao-op-item{display:flex;align-items:center;gap:8px;font-size:.78rem;color:var(--text-tertiary);margin-bottom:4px}
...
.ao-metric-label{font-size:.7rem;color:var(--text-dim)}
.ao-metric-value{font-size:1rem;font-weight:700;color:var(--text-primary)}
.ao-step-detail{background:var(--bg-elevated);padding:8px 10px;border-radius:6px;font-size:.78rem;color:var(--text-tertiary);font-family:'Fira Code',monospace}
.ao-array-state{background:var(--bg-elevated);padding:8px 10px;border-radius:6px;font-size:.72rem;color:var(--text-tertiary);font-family:'Fira Code',monospace;word-break:break-all;line-height:1.5}
.ao-array-length{font-size:.7rem;color:var(--text-dim);margin-top:4px}

/* Sections */
.ao-section{background:var(--bg-card);border: 1px solid var(--border-default);border-radius:12px;margin-bottom:12px;overflow:hidden}
.ao-section-toggle{width:100%;display:flex;align-items:center;gap:10px;padding:14px 18px;background:none;border:none;color:var(--text-secondary);font-size:1rem;font-weight:600;cursor:pointer;text-align:left;transition:background .15s}
.ao-section-toggle:hover{background:rgba(100,116,139,.1)}
.ao-info-circle{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:var(--accent-bg);border: 1px solid var(--accent-border);color:var(--accent-lighter);font-size:.82rem;font-weight:700;flex-shrink:0}
.ao-section-body{padding:0 20px 20px;color:var(--text-tertiary);font-size:.88rem;line-height:1.7}
.ao-section-body h2{font-size:1.15rem;color:var(--text-primary);margin:0 0 8px}
.ao-section-body h3{font-size:.95rem;color:var(--accent-lighter);margin:16px 0 6px}
.ao-section-body ol,.ao-section-body ul{padding-left:20px;margin:4px 0}
.ao-section-body li{margin-bottom:3px}
.ao-section-intro{color:var(--text-muted);margin:0 0 16px;font-size:.9rem}
.ao-how-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px}
.ao-how-card{background:var(--bg-input);border: 1px solid var(--border-default);border-radius:12px;padding:16px;display:flex;flex-direction:column;gap:12px;min-height:220px;transition:border .2s,transform .2s}
.ao-how-card.active{border-color: var(--accent-border);transform:translateY(-4px);box-shadow:0 8px 24px rgba(99,102,241,.2)}
.ao-how-header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}
.ao-how-header h3{margin:0;color:var(--text-primary);font-size:1rem}
.ao-how-header p{margin:4px 0 0;color:var(--text-tertiary);font-size:.85rem}
.ao-chip{background:var(--accent-bg);border: 1px solid var(--accent-border);border-radius:999px;padding:3px 10px;font-size:.7rem;color:var(--accent-lighter);font-weight:600}
.ao-step-list{margin:0;padding-left:18px;color:var(--text-tertiary);font-size:.84rem;line-height:1.5}
.ao-how-meta{display:flex;flex-wrap:wrap;gap:8px;font-size:.75rem;color:var(--text-muted)}
.ao-how-meta strong{color:var(--text-primary)}
.ao-edge-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px}
.ao-edge-card{background:var(--bg-elevated);border: 1px solid var(--border-default);border-radius:10px;padding:14px;cursor:pointer;transition:all .15s}
.ao-edge-card:hover{border-color: var(--accent-border);background:var(--accent-bg)}
.ao-edge-card strong{display:block;color:var(--text-primary);font-size:.88rem;margin-bottom:4px}
.ao-edge-card small{color:var(--text-dim);font-size:.78rem}
