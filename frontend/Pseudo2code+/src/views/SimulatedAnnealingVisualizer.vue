          ...
          <div class="bs-chart-wrapper">
            <div class="sa-viz-container">
              ...
              <svg :viewBox="`0 0 ${svgW} ${svgH}`" class="sa-svg" preserveAspectRatio="xMidYMid meet">
                <!-- Grid -->
                <line v-for="i in 11" :key="'gx'+i" :x1="sx(i-1)" :y1="marginT" :x2="sx(i-1)" :y2="marginT+plotH" stroke="var(--border-light)" stroke-width="1"/>
                <line v-for="j in 6" :key="'gy'+j" :x1="marginL" :y1="marginT+((j-1)/5)*plotH" :x2="marginL+plotW" :y2="marginT+((j-1)/5)*plotH" stroke="var(--border-light)" stroke-width="1"/>
                <!-- Axes -->
...
.bs-btn.active{background:var(--accent-bg);border-color: var(--accent-border)}
...
.bs-setting-row label{font-size:.78rem;color:var(--text-muted)}
...
.bs-legend-item{display:flex;gap:8px;align-items:flex-start;margin-bottom:8px;font-size:.78rem;color:var(--text-tertiary)}
...
.sa-temp-label{font-size:.78rem;color:var(--text-muted);white-space:nowrap;min-width:90px;text-align:right}
/* Status */
.sa-status-badge{padding:8px 18px;border-radius:10px;font-size:.82rem;font-weight:600;text-align:center;background:rgba(100,116,139,.1);border: 1px solid var(--border-default);color:var(--text-tertiary)}
...
.sa-sb-accepted_better{background:rgba(34,197,94,.08);border-color:rgba(34,197,94,.25);color: var(--success-text)}
.sa-sb-accepted_worse{background:rgba(234,179,8,.08);border-color:rgba(234,179,8,.25);color: var(--warning)}
.sa-sb-rejected{background:rgba(239,68,68,.08);border-color:rgba(239,68,68,.25);color: var(--error-text)}
.sa-sb-cooling{background:rgba(6,182,212,.08);border-color:rgba(6,182,212,.25);color:#67e8f9}
.sa-sb-success{background:rgba(139,92,246,.1);border-color:rgba(139,92,246,.3);color: var(--accent-light)}
/* Prob bar */
.sa-prob-row{display:flex;align-items:center;gap:8px;width:100%;max-width:420px}
.sa-prob-label{font-size:.72rem;color:var(--text-muted);white-space:nowrap}
.sa-prob-bar-track{flex:1;height:8px;background:rgba(100,116,139,.2);border-radius:4px;overflow:hidden}
.sa-prob-bar-fill{height:100%;background: var(--accent);border-radius:4px;transition:width .3s}
.sa-prob-val{font-size:.78rem;font-weight:700;color:var(--accent-lighter);font-family:'Fira Code',monospace;min-width:44px;text-align:right}
/* Inspector */
...
.sa-state-box{background:var(--bg-elevated);border-radius:8px;padding:8px 10px;display:flex;flex-direction:column;align-items:center}
.sa-st-lbl{font-size:.68rem;color:var(--text-dim);font-weight:600}.sa-st-val{font-size:1.1rem;font-weight:800;color:var(--text-primary);font-family:'Fira Code',monospace}
...
/* Decision box */
.sa-decision-box{padding:8px 12px;border-radius:8px;text-align:center;font-weight:700;font-size:.85rem;background:rgba(100,116,139,.1);color:var(--text-muted);border: 1px solid var(--border-default)}
.sa-dec-accepted_better{background:rgba(34,197,94,.08);color: var(--success-text);border-color:rgba(34,197,94,.25)}
.sa-dec-accepted_worse{background:rgba(234,179,8,.08);color: var(--warning);border-color:rgba(234,179,8,.25)}
.sa-dec-rejected{background:rgba(239,68,68,.08);color: var(--error-text);border-color:rgba(239,68,68,.25)}
/* History log */
.sa-history-log{background:var(--bg-elevated);padding:6px 8px;border-radius:6px;max-height:130px;overflow-y:auto}
...
.sa-hist-item{display:flex;gap:8px;align-items:center;font-size:.72rem;color:var(--text-muted);padding:2px 0}
.sa-hist-cur{color:var(--accent-lighter);font-weight:600}
.sa-hist-n{font-weight:700;color:var(--text-dim);font-family:'Fira Code',monospace;min-width:24px}
.sa-hist-e{color:var(--accent-light);font-family:'Fira Code',monospace}
.sa-hist-a{font-weight:700}.sa-ha-yes{color:#22c55e}.sa-ha-no{color:#ef4444}
.sa-no-hist{font-size:.72rem;color:var(--text-faint);font-style:italic}
/* Sections */
