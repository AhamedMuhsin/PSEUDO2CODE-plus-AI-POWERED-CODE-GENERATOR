<template>
    ...
    <main class="sv-page">
        <div class="sv-container">
            ...
            <h1 class="sv-title">Selection Sort</h1>

            <div class="sv-three-col">
                <!-- LEFT -->
                <aside class="sv-controls-panel">
                    <div class="sv-btn-group">
                        <button class="sv-btn" :class="{ active: playing }" @click="playing ? pause() : play()">
                            <span class="sv-icon">▶</span> {{ playing ? 'Pause' : 'Play' }}
                        </button>
                        <button class="sv-btn" @click="next" :disabled="stepIndex === steps.length - 1">
                            <span class="sv-icon">⏭</span> Step
                        </button>
                    </div>
                    <div class="sv-btn-group">
                    ...
                    <button class="sv-btn sv-settings-toggle" @click="showSettings = !showSettings">
                        <span class="sv-icon">⚙</span> Settings
                    </button>
                    <div v-if="showSettings" class="sv-settings-body">
                        <div class="sv-setting-row"><label>Size: <strong>{{ arraySize }}</strong></label>
                            <input type="range" min="5" max="30" v-model.number="arraySize" class="sv-slider" /></div>
                        <div class="sv-setting-row"><label>Speed: <strong>{{ speedPercent }}%</strong></label>
                            <input type="range" min="1" max="5" v-model.number="speedLevel" class="sv-slider" /></div>
                    </div>
                    <div class="sv-custom-input">
                        <label>Custom Array:</label>
                        <div class="sv-input-row">
                            <input v-model="customInput" placeholder="e.g. 5,2,8,1,9" class="sv-text-input" @keydown.enter="applyCustomArray" />
                            <button class="sv-btn sv-apply-btn" @click="applyCustomArray">Go</button>
                        </div>
                    </div>
                    ...
                    <div class="sv-shortcuts"><h4>Keyboard Shortcuts:</h4>
                        <div class="sv-shortcut-grid"><span class="sv-key">Space</span><span>Play/Pause</span><span class="sv-key">→</span><span>Step Forward</span><span class="sv-key">←</span><span>Step Back</span><span class="sv-key">R</span><span>Reset</span></div>
                    </div>
                ...
                <div class="sv-chart-area">
                    <div class="sv-chart-wrapper">
                        <div class="sv-bars-container">
                            <div v-for="(value, i) in currentStep.array" :key="i" class="sv-bar-col">
                                <div class="sv-bar" :class="{ comparing: currentStep.active.includes(i) && !currentStep.swap, swapping: currentStep.swap && currentStep.active.includes(i), sorted: (currentStep.sorted || []).includes(i) }" :style="{ height: (value / maxValue) * 100 + '%' }">
                                    <span class="sv-bar-label">{{ value }}</span>
                                </div>
                                <span class="sv-bar-index">{{ i }}</span>
                            </div>
                        </div>
                        <div class="sv-chart-footer"><span>Selection Sort Chart - {{ currentStep.array.length }} elements</span><span>Max value: {{ maxValue }}</span></div>
                    ...
                    <div class="sv-complexity-row">
                        <span class="sv-complexity-badge">Time Complexity: <strong>O(n²)</strong></span>
                        <span class="sv-complexity-badge">Space Complexity: <strong>O(1)</strong></span>
                    </div>
                </div>
                ...
                <aside class="sv-inspector">
                    ...
                    <div v-if="sortedCount > 0" class="sv-op-item"><span class="sv-op-dot sorted"></span>Sorted: first {{ sortedCount }} elements</div>
                    ...
                    <div class="sv-metrics-grid">
                        ...
                        <div class="sv-metric"><span class="sv-metric-label">Pass</span><span class="sv-metric-value">{{ metrics.pass }}</span></div>
            ...
            <section class="sv-section">
                <button class="sv-section-toggle" @click="showHowItWorks = !showHowItWorks"><span class="sv-info-circle">ⓘ</span> How Selection Sort Works</button>
                <div v-if="showHowItWorks" class="sv-section-body">
                    <h2>Algorithm Overview</h2>
                    <p>Selection Sort divides the array into a sorted and unsorted region. It repeatedly finds the minimum element from the unsorted part and moves it to the end of the sorted part.</p>
                    <h3>How it works:</h3>
                    <ol><li>Find the minimum element in the unsorted portion</li><li>Swap it with the first unsorted element</li><li>Move the boundary of the sorted region one element forward</li><li>Repeat until the entire array is sorted</li></ol>
                    <h3>Why "Selection" Sort?</h3>
                    <p>The algorithm is named because it works by repeatedly <strong>selecting</strong> the smallest (or largest) element from the unsorted portion and placing it in its correct position.</p>
                    <h3>Complexity Analysis:</h3>
                    <ul><li><strong>Best Case:</strong> O(n²) – always scans the entire unsorted portion</li><li><strong>Average Case:</strong> O(n²) – typical random data</li><li><strong>Worst Case:</strong> O(n²) – same number of comparisons regardless</li><li><strong>Space Complexity:</strong> O(1) – in-place sorting</li></ul>
                </div>
            </section>
            ...
            <section class="sv-section">
                ...
                <div v-if="showEdgeCases" class="sv-section-body">
                    ...
                    <div class="sv-edge-grid">
                        <div class="sv-edge-card" @click="loadEdgeCase([1,2,3,4,5])"><strong>Already Sorted: [1,2,3,4,5]</strong><small>Still performs all comparisons – O(n²)</small></div>
                        <div class="sv-edge-card" @click="loadEdgeCase([5,4,3,2,1])"><strong>Reverse Sorted: [5,4,3,2,1]</strong><small>Same number of comparisons as best case</small></div>
                        <div class="sv-edge-card" @click="loadEdgeCase([3,1,4,1,5,9,2,6,5])"><strong>Duplicates: [3,1,4,1,5,9,2,6,5]</strong><small>Selection sort is NOT stable – may change relative order</small></div>
                    ...
                    <ul class="sv-tips"><li>Selection sort is <strong>not stable</strong> – equal elements may change order</li><li>Makes the minimum number of swaps – O(n) swaps in worst case</li><li>In-place sorting – requires only O(1) additional memory</li><li>Simple implementation but inefficient for large datasets</li></ul>
                </div>
            </section>
        </div>
    </main>
...
const steps = ref(generateSelectionSortSteps(baseArray.value))
const stepIndex = ref(0)
const playing = ref(false)
const totalSteps = computed(() => steps.value.length)
const currentStepNumber = computed(() => stepIndex.value + 1)
const currentStep = computed(() => steps.value[stepIndex.value] || { array: [], active: [], swap: false, sorted: [], explanation: '' })
...
const metrics = computed(() => {
    let comparisons = 0, swaps = 0, pass = 0
    for (let i = 0; i <= stepIndex.value; i++) {
        const s = steps.value[i]
        if (s.explanation.startsWith('Compare')) comparisons++
        if (s.swap) swaps++
        if (s.explanation.startsWith('Assume') || s.explanation.startsWith('Pass') || s.explanation.startsWith('Start')) pass++
    }
    const n = currentStep.value.array.length
    const maxC = n * (n - 1) / 2
    const efficiency = maxC > 0 ? Math.round((1 - swaps / maxC) * 100) : 100
...
function reset() { pause(); baseArray.value = randomArray(); steps.value = generateSelectionSortSteps(baseArray.value); stepIndex.value = 0 }
function generateRandom() { reset() }
function applyCustomArray() { const arr = customInput.value.split(',').map(n => Number(n.trim())).filter(n => !isNaN(n) && n > 0); if (arr.length < 2) return; pause(); baseArray.value = arr; steps.value = generateSelectionSortSteps(arr); stepIndex.value = 0 }
function goToGenerateCode() { router.push({ path: '/generate-code', query: { prompt: 'Write a program for the Selection Sort algorithm.\nTake a random input array.' } }) }
function loadEdgeCase(arr) { pause(); baseArray.value = [...arr]; steps.value = generateSelectionSortSteps(baseArray.value); stepIndex.value = 0 }
watch(speedLevel, () => { if (playing.value) { pause(); play() } })
watch(arraySize, (n) => { pause(); baseArray.value = randomArray(n); steps.value = generateSelectionSortSteps(baseArray.value); stepIndex.value = 0 })
