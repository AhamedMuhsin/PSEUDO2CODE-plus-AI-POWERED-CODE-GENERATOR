<template>
    ...
    <main class="sv-page">
        <div class="sv-container">
            ...
            <h1 class="sv-title">Quick Sort</h1>
            ...
            <div class="sv-three-col">
                <aside class="sv-controls-panel">
                    ...
                    <div class="sv-legend"><h4>Legend</h4>
                        ...
                        <div class="sv-legend-item"><span class="sv-dot pivot"></span><div><strong>Pivot</strong><br/><small>Current pivot element</small></div></div>
                        <div class="sv-legend-item"><span class="sv-dot range"></span><div><strong>Range</strong><br/><small>Current partition bounds</small></div></div>
                ...
                <div class="sv-chart-area">
                    <div class="sv-chart-wrapper">
                        ...
                        <div class="sv-chart-footer"><span>Quick Sort Chart - {{ currentStep.array.length }} elements</span><span>Max value: {{ maxValue }}</span></div>
                    ...
                    <div class="sv-complexity-row"><span class="sv-complexity-badge">Time Complexity: <strong>O(n log n)</strong></span><span class="sv-complexity-badge">Space Complexity: <strong>O(log n)</strong></span></div>
                </div>
                ...
                <aside class="sv-inspector">
                    ...
                    <div v-if="currentStep.pivot != null" class="sv-op-item"><span class="sv-op-dot pivot"></span>Pivot index: {{ currentStep.pivot }}</div>
                    ...
                    <div class="sv-metrics-grid">
                        ...
                        <div class="sv-metric"><span class="sv-metric-label">Partitions</span><span class="sv-metric-value">{{ metrics.partitions }}</span></div>
            ...
            <section class="sv-section">
                <button class="sv-section-toggle" @click="showHowItWorks = !showHowItWorks"><span class="sv-info-circle">ⓘ</span> How Quick Sort Works</button>
                <div v-if="showHowItWorks" class="sv-section-body">
                    <h2>Algorithm Overview</h2>
                    <p>Quick Sort is a highly efficient divide-and-conquer algorithm. It selects a "pivot" element and partitions the array so that smaller elements go left and larger elements go right, then recursively sorts each side.</p>
                    <h3>How it works:</h3>
                    <ol><li>Choose a pivot element from the array</li><li>Partition: rearrange elements so that values less than the pivot go left and greater go right</li><li>Recursively apply Quick Sort to the left and right subarrays</li><li>The array is sorted when all partitions are of size 0 or 1</li></ol>
                    <h3>Why "Quick" Sort?</h3>
                    <p>On average, Quick Sort is one of the <strong>fastest</strong> general-purpose sorting algorithms, performing well due to good cache locality and low constant factors.</p>
                    <h3>Complexity Analysis:</h3>
                    <ul><li><strong>Best Case:</strong> O(n log n) – balanced partitions</li><li><strong>Average Case:</strong> O(n log n)</li><li><strong>Worst Case:</strong> O(n²) – when pivot is always the smallest or largest element</li><li><strong>Space Complexity:</strong> O(log n) – call stack depth</li></ul>
                </div>
            </section>
            ...
            <section class="sv-section">
                ...
                <div v-if="showEdgeCases" class="sv-section-body">
                    ...
                    <div class="sv-edge-grid">
                        <div class="sv-edge-card" @click="loadEdgeCase([1,2,3,4,5])"><strong>Already Sorted: [1,2,3,4,5]</strong><small>Can trigger worst case O(n²) depending on pivot strategy</small></div>
                        <div class="sv-edge-card" @click="loadEdgeCase([5,4,3,2,1])"><strong>Reverse Sorted: [5,4,3,2,1]</strong><small>Also worst case for naive pivot selection</small></div>
                        <div class="sv-edge-card" @click="loadEdgeCase([3,1,4,1,5,9,2,6,5])"><strong>Duplicates: [3,1,4,1,5,9,2,6,5]</strong><small>Quick sort is NOT stable – duplicates may reorder</small></div>
                        <div class="sv-edge-card" @click="loadEdgeCase([42])"><strong>Single Element: [42]</strong><small>Base case – already sorted</small></div>
                    </div>
                    <h3>Performance Tips:</h3>
                    <ul class="sv-tips"><li>Quick sort is <strong>NOT stable</strong> – equal elements may change relative order</li><li>In-place sorting – requires only O(log n) additional space for the call stack</li><li>Pivot selection matters – random or median-of-three avoids worst case</li><li>Often the fastest in practice for large random datasets</li></ul>
                </div>
            </section>
        </div>
    </main>
...
const steps = ref(generateQuickSortSteps(baseArray.value))
...
const metrics = computed(() => {
    ...
    for (let i = 0; i <= stepIndex.value; i++) {
        ...
        if (s.swap) swaps++
        if (s.explanation.includes('Pivot') || s.explanation.includes('pivot') || s.explanation.includes('Partition') || s.explanation.includes('partition')) partitions++
    }
    const n = currentStep.value.array.length
    const maxC = n * (n - 1) / 2
    const efficiency = maxC > 0 ? Math.round((1 - comparisons / maxC) * 100) : 100
    return { comparisons, swaps, partitions, efficiency }
})
...
function reset() { pause(); baseArray.value = randomArray(); steps.value = generateQuickSortSteps(baseArray.value); stepIndex.value = 0 }
function generateRandom() { reset() }
function applyCustomArray() { const arr = customInput.value.split(',').map(n => Number(n.trim())).filter(n => !isNaN(n) && n > 0); if (arr.length < 2) return; pause(); baseArray.value = arr; steps.value = generateQuickSortSteps(arr); stepIndex.value = 0 }
function goToGenerateCode() { router.push({ path: '/generate-code', query: { prompt: 'Write a program for the Quick Sort algorithm.\nTake a random input array.' } }) }
function loadEdgeCase(arr) { pause(); baseArray.value = [...arr]; steps.value = generateQuickSortSteps(baseArray.value); stepIndex.value = 0 }
watch(speedLevel, () => { if (playing.value) { pause(); play() } })
watch(arraySize, (n) => { pause(); baseArray.value = randomArray(n); steps.value = generateQuickSortSteps(baseArray.value); stepIndex.value = 0 })
