<template>
    ...
    <main class="sv-page">
        <div class="sv-container">
            ...
            <h1 class="sv-title">Insertion Sort</h1>
            ...
            <div class="sv-three-col">
                ...
                <div class="sv-chart-area">
                    <div class="sv-chart-wrapper">
                        ...
                        <div class="sv-chart-footer"><span>Insertion Sort Chart - {{ currentStep.array.length }} elements</span><span>Max value: {{ maxValue }}</span></div>
                    ...
                    <div class="sv-complexity-row"><span class="sv-complexity-badge">Time Complexity: <strong>O(n²)</strong></span><span class="sv-complexity-badge">Space Complexity: <strong>O(1)</strong></span></div>
                </div>
                ...
                <aside class="sv-inspector">
                    ...
                    <div class="sv-metrics-grid">
                        ...
                        <div class="sv-metric"><span class="sv-metric-label">Shifts</span><span class="sv-metric-value">{{ metrics.swaps }}</span></div>
                        <div class="sv-metric"><span class="sv-metric-label">Insertions</span><span class="sv-metric-value">{{ metrics.pass }}</span></div>
            ...
            <section class="sv-section">
                <button class="sv-section-toggle" @click="showHowItWorks = !showHowItWorks"><span class="sv-info-circle">ⓘ</span> How Insertion Sort Works</button>
                <div v-if="showHowItWorks" class="sv-section-body">
                    <h2>Algorithm Overview</h2>
                    <p>Insertion Sort builds the final sorted array one element at a time. It picks each element and inserts it into its correct position among the previously sorted elements, much like sorting a hand of playing cards.</p>
                    <h3>How it works:</h3>
                    <ol><li>Start from the second element (index 1)</li><li>Compare it with the elements before it</li><li>Shift all larger elements one position to the right</li><li>Insert the element into its correct position</li><li>Repeat for all remaining elements</li></ol>
                    <h3>Why "Insertion" Sort?</h3>
                    <p>The algorithm is named because it works by <strong>inserting</strong> each new element into the correct position within the already-sorted portion of the array.</p>
                    <h3>Complexity Analysis:</h3>
                    <ul><li><strong>Best Case:</strong> O(n) – when the array is already sorted</li><li><strong>Average Case:</strong> O(n²) – typical random data</li><li><strong>Worst Case:</strong> O(n²) – when the array is reverse sorted</li><li><strong>Space Complexity:</strong> O(1) – in-place sorting</li></ul>
                </div>
            </section>
            ...
            <section class="sv-section">
                ...
                <div v-if="showEdgeCases" class="sv-section-body">
                    ...
                    <div class="sv-edge-grid">
                        <div class="sv-edge-card" @click="loadEdgeCase([1,2,3,4,5])"><strong>Already Sorted: [1,2,3,4,5]</strong><small>Best case – O(n) with zero shifts</small></div>
                        <div class="sv-edge-card" @click="loadEdgeCase([5,4,3,2,1])"><strong>Reverse Sorted: [5,4,3,2,1]</strong><small>Worst case – maximum shifts required</small></div>
                        <div class="sv-edge-card" @click="loadEdgeCase([3,1,4,1,5,9,2,6,5])"><strong>Duplicates: [3,1,4,1,5,9,2,6,5]</strong><small>Insertion sort is stable – maintains relative order</small></div>
                    ...
                    <ul class="sv-tips"><li>Insertion sort is <strong>stable</strong> – equal elements maintain their relative order</li><li>Adaptive – performs well on partially sorted data</li><li>In-place sorting – requires only O(1) additional memory</li><li>Efficient for small datasets – often used as base case in hybrid sorts</li></ul>
                </div>
            </section>
        </div>
    </main>
...
const steps = ref(generateInsertionSortSteps(baseArray.value))
...
const metrics = computed(() => {
    ...
    for (let i = 0; i <= stepIndex.value; i++) {
        ...
        if (s.explanation.startsWith('Compare') || s.explanation.startsWith('Check')) comparisons++
        if (s.swap || s.explanation.includes('shift') || s.explanation.includes('Shift') || s.explanation.includes('Move')) swaps++
        if (s.explanation.startsWith('Select') || s.explanation.startsWith('Insert') || s.explanation.startsWith('Pick')) pass++
    }
...
function reset() { pause(); baseArray.value = randomArray(); steps.value = generateInsertionSortSteps(baseArray.value); stepIndex.value = 0 }
function generateRandom() { reset() }
function applyCustomArray() { const arr = customInput.value.split(',').map(n => Number(n.trim())).filter(n => !isNaN(n) && n > 0); if (arr.length < 2) return; pause(); baseArray.value = arr; steps.value = generateInsertionSortSteps(arr); stepIndex.value = 0 }
function goToGenerateCode() { router.push({ path: '/generate-code', query: { prompt: 'Write a program for the Insertion Sort algorithm.\nTake a random input array.' } }) }
function loadEdgeCase(arr) { pause(); baseArray.value = [...arr]; steps.value = generateInsertionSortSteps(baseArray.value); stepIndex.value = 0 }
watch(speedLevel, () => { if (playing.value) { pause(); play() } })
watch(arraySize, (n) => { pause(); baseArray.value = randomArray(n); steps.value = generateInsertionSortSteps(baseArray.value); stepIndex.value = 0 })
